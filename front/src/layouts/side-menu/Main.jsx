...
function Main() {
  ...
  useEffect(()=> {
    if(!isVerified){
      ...
      setIsLoading(false);
      if(verificationResult.user.isAdmin) {
        setUserRole(1);
      }else {
        setUserRole(2);
      }
  ...
  return (
    !isLoading && <div className="py-5 md:py-0 -mx-3 px-3 sm:-mx-8 sm:px-8 bg-black/[0.15] dark:bg-transparent">
      <DarkModeSwitcher />
      <MainColorSwitcher />
      <MobileMenu />
            ...
            {formattedMenu.map((menu, menuKey) =>
              menu == "devider" ? (
                <li
                  ...
                  key={menu + menuKey}
                ></li>
              ) : menu.user && (menu.user == userRole || menu.user == 3)? (
                <li key={menu + menuKey}>
                  <SideMenuTooltip
                    tag="a"
                    content={menu.title}
                    ...
                    >
                      ...
                      >
                        {menu.subMenu.map((subMenu, subMenuKey) => (
                          subMenu.user && (subMenu.user == userRole || subMenu.user == 3)?<li key={subMenuKey}>
                            <SideMenuTooltip
                              tag="a"
